...
Encore
  ...
  .setPublicPath("/themes/drupaleurope/build")
...
if (
  ...
  config.module.rules
    .filter(
      (rule) =>
        rule.test.toString() === /\.(png|jpg|jpeg|gif|ico|svg|webp)$/.toString()
    )
    .map((rule) => {
  ...
  let svgRule = Object.assign(
    {},
    config.module.rules.filter(
      (rule) =>
        rule.test.toString() === /\.(png|jpg|jpeg|gif|ico|webp)$/.toString()
    )[0]
  );
  svgRule.test = /\.(svg)$/;
  svgRule.use = [
